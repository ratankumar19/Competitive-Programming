...
int countSize(char cake[NMAX][NMAX], int x, int y, int n, bool **isVisited){
    isVisited[x][y] = true;
    int total = 1;//becz phla barrr 1 mil jayega to uske liye total 
    for(int i = 0; i < 4; i++){
        int nextX = x+moveX[i];
        int nextY = y+moveY[i];
    ...
    for(int i = 0; i < n; i++){
        for(int j = 0; j < n; j++){
            if(cake[i][j] == '1' && !isVisited[i][j]){
                maxSize = max(maxSize, countSize(cake, i, j, n, isVisited));//update walaa code h
            }
        }
    }
    //isme visit jo ho gyaa to phir usko unmarked nii krna hai
